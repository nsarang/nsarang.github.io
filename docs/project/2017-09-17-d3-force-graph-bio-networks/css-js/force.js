...
d3.json("data/motif.json", function (error, read) {
  ...
  d3.select("#chargeINPUT").property("value", nNodeCharge).dispatch("input");
  d3.select("#linkStrINPUT").property("value", nLinkStr).dispatch("input");
  d3.select("#nodeRadINPUT").property("value", nNodeRad).dispatch("input");
  d3.select("#linkWidthINPUT").property("value", nLinkWid).dispatch("input");
  d3.select("#veloDecayINPUT").property("value", nVelDecay).dispatch("input");
  d3.select("#centerGravityINPUT").property("value", nCenterGrav).dispatch("input");
});
...
function showInfo(key = null) {
  if (!info[key]) {
    if (d3.event.type === "mouseout") {
      key = "about";
    } else if (this.tagName === "circle") {
      key = "highlight";
    } else if (this.id === "forceHandle") {
      key = "forceLabel";
    } else {
      key = this.id;
    }
  ...
  // Use a closure or an IIFE to store the previous key
  showInfo.previousKey = showInfo.previousKey || null;
  if (showInfo.previousKey === key) {
    return;
  }

  // Check if info[key] exists
  if (info[key]) {
    d3.select('[data-anchor-id="infoTitle"]').text(info[key].title);
    d3.select("#infoDef").html(info[key].def);
  } else {
    console.warn(`No info found for key: ${key}`);
  }

  showInfo.previousKey = key;
}